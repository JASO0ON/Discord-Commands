...
module.exports = {
    ...
    .addSubcommand(command => command.setName('list').setDescription('Lists all servers PixelVal is in.')),
    /**    
    ...
    async execute(interaction, client) {
        ...
        interaction.reply({ embeds: [
            ...
            let guilds = await client.guilds.fetch()
                ...
                .setTitle(`> PixelVal is in ${client.guilds.cache.size} Servers`)
                .setTimestamp()
                .setThumbnail('https://cdn.discordapp.com/icons/1078641070180675665/c3ee76cdd52c2bba8492027dfaafa15d.webp?size=1024')
                .setColor("DarkRed")
                .setDescription(guilds.map(g => `- ${g.name} \n> **ID:** ${g.id}`).join("\n").slice(0, 4000))
                .setFooter({ text: "📃 Server List Displayed"})
                .setAuthor({ name: `📃 Server List`})
